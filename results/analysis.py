...
# TODO: what plots to include, bounds are so tight that hard to express visually.
pert = list((before[nt]["PERT"] / before[nt]["MCN"][-1].mu)*100 for nt in n_tasks)
print(pert)
...
ax1.bar(x, list((before[nt]["PERT"] / before[nt]["MCN"][-1].mu)*100 for nt in n_tasks), width=0.95, color='#E24A33', edgecolor='white')
ax1.set_xlabel("DAG SIZE", labelpad=5)
ax1.set_ylabel("% OF EXPECTED VALUE", labelpad=5)
# ax1.legend(handlelength=3, handletextpad=0.4, ncol=2, loc='best', fancybox=True, facecolor='white') 
plt.savefig('{}/bounds'.format(plot_path), bbox_inches='tight') 
plt.close(fig) 
