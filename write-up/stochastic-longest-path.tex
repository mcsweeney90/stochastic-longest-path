...
%(As an aside, Van Slyke's analysis, albeit only rough, does not depend at all on the size of the graph, which seems counterintuitive. There are other approaches as well, but they also largely suffer from this apparent issue...)  
...
\begin{table}
	...
	\end{center}	
      ...
      Leaving aside the question of how close to normal the longest path distributions actually are, perhaps the more important takeaway from the table and the figure is that they appear to be very similar for both choices of weight distribution, as we would hope. It should however also be emphasized that they aren't identical. This means there will always be a range---however narrow---of distributions that the ``true'' solution can take under the assumption that only the first two moments of the weight distributions are known.  
      % should we quantify how similar the two distributions are?
...
Several bounds on both the moments and distribution function of the longest path were discussed in Section \ref{sect.bounds}. Given the conclusions of the previous section and other, more rigorous empirical studies \cite{can16,lud01}, we assume that the longest path distribution is at least roughly normal, so we are most interested in the former. Our experience in the previous chapter with Fulkerson's bound on the expected value suggests that in a scheduling context that approach can be impractically expensive, so we focus here largely on Kamburowski's bounds, since they have received less attention in the literature.

Although the bounds technically only hold for normally distributed weights, the CLT suggests that they may still be useful heuristically when that is not the case, since the longest path distribution is likely to be similar in any event. The closeness of the empirical distributions for Cholesky graphs with normal and Gamma weights in the previous section also supports this. For the remainder of this section we will assume that all weights are normal, but this should always be borne in mind.
...
\begin{table}
	...
	\label{plot.variance_bounds}
      ...
      As well as the tightness of the bounds, the other key consideration is how long they take to compute. As always, this depends to an extent on the details of the implementation but for what it's worth, in Figure X we display the runtime for our own (with the same setup used throughout this section). Note that the mean bounds are much more expensive to compute than the variance since they require repeated evaluation of the unit normal cdf. Furthermore, the mean bounds depend on the variance but not vice versa, so if only the variance is required then the runtime would be considerably reduced. 
      ...
      Of the three correlation-aware heuristics considered by Canon and Jeannot, CorLCA arguably achieved the best balance of speed and accuracy, so we consider only that as an alternative to Sculli's method here. Now, since both CorLCA and Sculli's method assume that the longest path distribution is normal, in order to evaluate the quality of their solutions we compare the computed moments with the corresponding statistics derived from the empirical distributions as described in Section \ref{subsect.empirical_distribution}. For the Cholesky DAG set, we found this to very much be a tale of two moments: for both heuristics, the purported means were very close to the true values, whereas the variance estimates were of much poorer quality, especially for the larger DAGs. With regards to their relative performance, CorLCA was usually, but not always, superior to Sculli's method for both moments. Table X summarizes all of the relevant data, including timings...

      Another observation we made that as far as we know has not been remarked on elsewhere is that the longest path estimates computed by the forward and backward variants of the two heuristics could vary considerably. For example, we found that for Cholesky schedule DAGs, the variance as computed by both Sculli's method and CorLCA was always smaller in the backward case. We attribute this to the fact that the average number of task children exceeds the average number of task parents so more pairwise maximizations are performed when moving forward and the effect of \eqref{eq.clark_max_sigma} is to decrease the variance, this may not be entirely surprising...
