...
Rather than just the moments, we may be more interested in bounds on the cumulative distribution function (cdf) $F_{L_n}$ of the longest path, where the bounds indicate (first-order) {\em stochastically dominant} relationships between the underlying RVs. In particular, we say that $B$ stochastically dominates $L_n$ if $F_{L_n}(z) = \P[L_n \leq z] \leq \P[B \leq z] = F_B(z) \; \forall z$. For convenience, from now on we simply write $F_{L_n} \leq F_B$ to represent the previous expression, so that the aim here is to determine $F_b$ and $F_B$ such that $F_b \leq F_{L_n} \leq F_B$. Sums and maximizations of random variables correspond to convolutions and products of their respective cdfs, which are easy to compute when the variables are independent. In particular,...

The first bounds of this type were proven by Kleindorfer, who gave both upper and lower bounds. The upper bound is computed by simply treating all of the operands of a maximization as independent and computing their product using (eqref). The lower is derived by simply selecting the minimal operand and using that... Inspired by the observation that Kleindorfer's upper bound is exact for series-parallel graphs, Dodin improved it in the general case by combining the method with a series of reductions...
...
%All of the known bounds of this type are based on first reducing the stochastic graph to series-parallel form so that maximizations are only ever performed on independent random variables. Bounding distributions are then computed through a series of convolutions and products on the weight cdfs. In this manner, Kleindorfer proved both upper and lower bounds on the cdf  \cite{kle71}, the first of which was tightened by Dodin \cite{dod85}.

%Reference to Bein? What about Ludwig? Shogan?
