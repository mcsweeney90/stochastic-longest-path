...
\begin{table}
	...
	\begin{center}	
		\begin{tabular}{c c c c c c c c c c c c}
                  ...
                  Routine & $\mu$ & $\sigma^2$ && $\mu$ & $\sigma^2$ && $\mu$ & $\sigma^2$ && $\mu$ & $\sigma^2$\\
                  \cmidrule{1-12}
                  {\tt GEMM} \\
                  {\tt POTRF} \\
                  {\tt SYRK} \\
...
Since the schedule determines which processor each task is executed on, the node and edge weights of the schedule graph are modeled as random variables with means and variances given by the sample means and variances observed in testing performed on a local accelerated system (as described in Chapter 3). Recall that the Cholesky factorization we consider here comprises tasks that represent one of four BLAS (or LAPACK) routines, {\tt GEMM}, {\tt POTRF}, {\tt SYRK} or {\tt TRSM}. The nodes of the schedule graph correspond to these tasks, so the weights are determined by two additional variables: $nb$, the tile size (either $128$ or $1024$ as before) and which the type (CPU or GPU) of the processor the schedule dictates should execute that task.   


with tile size $128$ and no asynchronous data transfers being assumed. Note that we make no assumptions here as to what distributions the weights follow, only that their first two moments are known.  
% not explained well.
	...
	\begin{center}	
		\begin{tabular}{c c c c c c c c c}
			...
			CPM & $99.5$ & $98.9$ & $98.5$ & $98.7$ & $99.2$ & $99.2$ & $99.4$ & $99.5$\\
                  ...
                  MC-10 & $100.0$ & $100.0$ & $99.8$ & $100.1$ & $99.9$ & $99.9$ & $99.9$ & $99.8$\\ % TODO. Remove entire table (except K bounds)?
			\bottomrule
		\end{tabular}
