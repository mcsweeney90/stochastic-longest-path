...
Fundamentally, the length of any given path through a schedule graph is just the sum of the weight RVs along it. By the Central Limit Theorem (CLT), sums of random variables are asymptotically normally distributed. So we can make a reasonable argument that longest path distribution itself is likely to be at least approximately normal, $L_n \approx N(\mu_n, \sigma_n)$. Indeed, this has often been observed empirically, even when the individual graph weights follow very different distributions \cite{can10}. This argument forms the basis of a family of efficient heuristics for computing an approximation to the longest path distribution.
...
\end{align}
Although these formulae are exact, they are only valid for a single pair of normal RVs. As already noted, the maximization of two normal RVs is not normal, so we cannot obtain the exact moments for a set of arbitrarily many RVs by applying them in a pairwise manner; we can however at least get an approximation. Altogether this means that by using \eqref{eq.sum_moments} for summations, and \eqref{eq.clark_max_mu} and \eqref{eq.clark_max_sigma} pairwise for maximizations, we can now move through the graph in a manner similar to \eqref{eq.Li} and compute approximations $\mu_n$ and $\sigma_n$ to the first two moments of the longest path which, since it is assumed to be roughly normal, suffices to describe the entire distribution. This method appears to have first been proposed for estimating the completion time of PERT networks by Sculli \cite{scu83}, although to simplify the problem he assumed that all of the correlation coefficients $\rho_{\epsilon \eta}$ in \eqref{eq.alpha_beta} were zero (see next section).

In practice, the moment estimates obtained using Sculli's method tend to be fairly good, with performance improving as the weight distributions move closer to normality and the number of nodes in the graph increases (as we might intuitively expect). More importantly, Sculli's method is typically much faster than the alternatives \cite{can16}. 
...
\begin{align}
  ...
  \end{align}
  The obvious problem with this method of computing the longest path is that $q$ is (usually) impractically large for even small graphs, so the dynamic programming approach defined by \eqref{eq.Li} is (usually) the only practical option. Hence \eqref{eq.first_principles} would seem to be of very little use in bounding or even approximating the longest path. However, by also making use of the normality assumption induced by the CLT, there are a handful of results that may be useful heuristically. The majority of these can also be applied to the smaller maximizations encountered in \eqref{eq.Li} but the hope is that by reducing the number of maximizations that need to be done we may be able to reduce the approximation error.  
  ...
  The best approach is to give up on exact bounds and take a heuristic approach...



An alternative bound on the cdf can perhaps be derived from the following result... The problem is that we don't know the mean, but there are good heuristic methods that usually give good approximations.
...
% sin07 and ross03 discuss the order in which the pairwise Clark approximations should be done. 
