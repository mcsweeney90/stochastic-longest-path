...
Although there are many different bounds on the longest path distribution in the literature, given our overriding interest in practical scheduling methods and our experience in the previous chapter with Fulkerson's bounds on the expected value, we eschew methods based on graph reductions or which require the evaluation of complex integrals and focus here largely bounds that can be computed through simple numerical schemes. In particular, Kamburowski's bounds on the mean and variance when costs are normally distributed are fairly cheap and only require the evaluation of the unit normal cdf, which is a standard part of most good numerical software libraries, so we largely focus on those. Basically we want to establish how tight the bounds are in practice.

Our conclusions are that while the lower bound on the mean is fairly tight, it is rarely a significant improvement on the classic (cheaper) PERT bound. The upper bound is much looser and rarely likely to be useful. As to the variance, both bounds are so loose as to be impractical...
