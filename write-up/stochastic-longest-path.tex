...
\subsection{Updating approximations}
\label{subsect.updating}

% Makespan or longest path? Have used LP earlier but makespan more sensible here...

The problem of approximating the longest path distribution of a stochastic graph has been widely-studied, as illustrated by the structure of this chapter so far. However a related problem has received less attention: how can estimated longest path distributions be quickly updated as weight realizations become apparent? From a scheduling perspective, this is equivalent to asking, how can makespan estimates be updated {\em during} execution? This would be particularly useful in situations such as when an unexpected delay occurs. More precisely, the scenario we consider is as follows. We have a complete set of longest path estimates $L_i \approx N(\mu_i, \sigma_i^2)$ for all nodes $i = 1, \dots, n$. At some point in time $T$, some fraction of the weights have been realized so that longest path estimates corresponding to some subset $C_T$ of the nodes have likewise been realized. How do we do update the longest path distribution $L_n$ accordingly? 


In conjunction with the path normality assumption induced by the CLT, the following proposition may be useful.
\begin{prop}
